...
keep if _merge == 3 // 55 districts from full sample

*cloning the residentleaweight var into a res_lea14 to be able to merge in step 2
generate res_lea14 = residentlea_weight  

*69 dups deleted
sort ric year grade_fnl bcode_weight dcode_weight isd_weight

keep ric res_lea14  residentlea_weight sample_27 sample_55 

save "${flint}\data\temp\fixed_rics_2014_55districts.dta", replace

***********************************************************************************
*************************************Step 2****************************************
*Creating a master list of rics based on the 60 districts that we want to examine.
...
*generating the residentlea 14 variable to be able to merge with the fixed rics above
gen res_lea14 = residentlea_weight if year == 2014
egen res_lea14_ = mode(res_lea14), by(ric) 
drop res_lea14
rename res_lea14_ res_lea14


merge m:1 ric res_lea14 using "${flint}\data\temp\fixed_rics_2014_55districts.dta"
keep if grade_fnl < 13 & year >= 2003
keep if _merge == 3 
drop if missing(residentlea_weight)

keep ric year grade_fnl bcode_weight dcode_weight isd_weight res_lea14 residentlea_weight sample_27 sample_55

save "${flint}\data\temp\fixed_rics_by_year_basedon55.dta", replace

***********************************************************************************
*************************************Step 3****************************************
*Creating a basic demographic file with student-level information based on the
*rics that lived in one of the 60 residential LEAs taht we created above.
***********************************************************************************
***********************************************************************************
...
merge 1:1 ric year using "${flint}\data\temp\fixed_rics_by_year_basedon55.dta"
keep if _merge == 3
...
*note that discipline variables are only available starting in 2012. 
...
*NEW ADDITION TO MAKE IT SIMILAR TO THE MERGE BY RESIDENTLEA OF STEP 3 OF THE NON-FIXED
merge 1:1 ric year using "${flint}\data\temp\fixed_rics_by_year_basedon55.dta"
keep if _merge == 3
drop _merge

save "${flint}\data\temp\srsd_fixed_55.dta", replace

***********************************************************************************
*************************************Step 4****************************************
*Creating the assessment file based on the rics that lived in one of the 60 residential 
*LEAs taht we created above.
***********************************************************************************
***********************************************************************************

use "${flint}\data\raw\assessment_standard_student_year.dta", clear

merge m:1 ric year using "${flint}\data\temp\fixed_rics_by_year_basedon55.dta"
keep if _merge == 3
drop _merge
...
*Creating a balanced panel so that all students have 15 rows (year 2003 to year 2017)
tsset ric year
tsfill, full
...
merge 1:1 ric year using "${flint}\data\raw\assessment_readiness_student_year.dta"
drop if _merge == 2
...
save "${flint}\data\temp\assessment_fixed_55.dta", replace

***********************************************************************************
*************************************Step 5****************************************
*Creating the school-level file based and then merging this info by bcode and year 
*based on the srds file that only has the rics from the 58 residential LEAs
***********************************************************************************
***********************************************************************************
...
use "${flint}\data\temp\srsd_fixed_55.dta" 
...
merge 1:1 ric year using "${flint}\data\temp\assessment_fixed_55.dta"
drop _merge
**ADDITIONS BASED ON NON-FIXED SAMPLE
merge 1:1 ric year using "${flint}\data\temp\srsd_fixed_55.dta" 
keep if _merge == 3
drop _merge
...
save "${flint}\data\temp\databeforeflags_fixed_55.dta", replace
...
***********************************************************************************
***********************************************************************************
use "${flint}\data\temp\databeforeflags_fixed_55.dta", clear
...
save "${flint}\data\student_level_fixed_55districts.dta", replace
